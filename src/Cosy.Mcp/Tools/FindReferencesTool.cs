...
public sealed class FindReferencesTool
{
    ...
    public async Task<object> FindAsync(
        ...
        [Description("Symbol to locate — DocumentationCommentId (preferred, e.g. M:Ns.Type.Method(System.String)) " +
            "or a partial name that the fuzzy resolver can route.")] string? symbol = null,
        ...
        CancellationToken ct = default)
    {
        ...
        logger.LogDebug("find_references: symbol={Symbol}, max={Max}, timeoutMs={TimeoutMs}", symbolText, max, timeoutMs);
        ...
        try
        {
            ...
            switch (resolved)
            {
                ...
                case ResolveResult.AmbiguousMatches a:
                    logger.LogWarning("find_references: ambiguous symbol: {Symbol} ({Count} candidates)", symbolText, a.TopN.Count);
